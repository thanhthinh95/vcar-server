var eventPage = function($) {
    ...
    function bindEventClick() {
        ...
        $(document).on('click', '#create_new', function (e) {
            _bindModalInfo(null, '#modal_info', _fields, ['name', 'urlDefault', 'description']);
        })

        $(document).on('click', '#edit_row_table', function (e) {
            var dataRow = _.find(dataTableRows, {_id : $(this).attr('data_id')});
            if(dataRow){
                _bindModalInfo(dataRow, '#modal_info', _fields, ['name', 'urlDefault', 'description']);             
            }else{
        ...
        $(document).on('submit', '#form_modal_info', function (e) {
            ...
            if(_.isEqual($(this).attr('data_action'), 'create')){
                _AjaxObject('/role', 'POST', objData, function(resp) {
                    if(resp.code == 200){
                ...
                _AjaxObject('/role', 'PUT', objData, function(resp) {
                    if(resp.code == 200){
        ...
        $(document).on('click', '#delete_row_table', function (e) {
            ...
            _DialogQuestion('Bạn đã chắc chắn ?', 'Quyền truy cập người dùng sẽ không còn vai trò này nữa', function () {
                _AjaxObject('/role', 'DELETE', {ids : [_id]}, function(resp) {
                    if(resp.code == 200){
        ...
        $(document).on('click', '#delete_item_checked', function (e) {
            ...
            if(_.has(dataIds, 'checkBoxIds')){
                _DialogQuestion('Bạn có chắc chắn?', 'Dữ liệu bạn chọn sẽ bị xóa bỏ vĩnh viễn', function () {
                    _AjaxObject('/role', 'DELETE', {ids : dataIds.checkBoxIds}, function(resp) {
                        if(resp.code == 200){
                            _DialogSuccess('Đã xóa bỏ thành công', function () {
                                _bindMenuSideBar(roleIndex._id);                    
                                _loadPageChild('role');
    ...
    function bindBodyTable(page) {
        ...
        _AjaxObject('/role/search', 'GET', objFilter, function(resp) {
            dataTableRows = null;
            if(resp.code == 200){
    ...
    return {
        init : function () {
            console.log('dang thuc hien init su kien role');
        ...
        uncut : function (){
            console.log('dang thuc hien uncut su kien role');
