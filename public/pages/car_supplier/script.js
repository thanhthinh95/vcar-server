var eventPage = function($) {
    ...
    function bindEventClick() {
        ...
        $(document).on('click', '#create_new', function (e) {
            _bindModalInfo(null, '#modal_info', _fields, ['name', 'numberPhone',  'startPoint', 'endPoint', 'status']);
        })

        $(document).on('click', '#edit_row_table', function (e) {
            var dataRow = _.find(dataTableRows, {_id : $(this).attr('data_id')});
            if(dataRow){
                 _bindModalInfo(dataRow, '#modal_info', _fields, ['name', 'numberPhone',  'startPoint', 'endPoint', 'status']);
            }else{
        ...
        $(document).on('submit', '#form_modal_info', function (e) {
            ...
            if(_.isEqual($(this).attr('data_action'), 'create')){
                _AjaxObject('/car_supplier', 'POST', objData, function(resp) {
                    if(resp.code == 200){
                ...
                _AjaxObject('/car_supplier', 'PUT', objData, function(resp) {
                    if(resp.code == 200){
    ...
    function bindBodyTable(page) {
        ...
        _AjaxObject('/car_supplier/search', 'GET', objFilter, function(resp) {
            dataTableRows = null;
            if(resp.code == 200){
    ...
    return {
        init : function () {
            console.log('dang thuc hien init su kien car_supplier');
        ...
        uncut : function (){
            console.log('dang thuc hien uncut su kien car_supplier');
