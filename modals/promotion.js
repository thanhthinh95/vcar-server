...
objSchame.statics._getAllAPI = async function (aggs) {
    return await _promotion.aggregate(aggs).allowDiskUse(true);  
}


objSchame.statics._getForCodeAPI = async function (aggs) {
    return await _promotion.aggregate(aggs).allowDiskUse(true);  
}
